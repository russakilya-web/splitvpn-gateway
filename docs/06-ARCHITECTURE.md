...
└─────────────────────────────────────────────────────────────────────┘
         ...
         └──────────────────────┬───────────────────────────┘
                                │ LAN ($LAN_IF)
                                │ $LAN_IP (по умолчанию 192.168.1.1/24)
                                │
         ┌──────────────────────▼───────────────────────────┐
         │                 ЛОКАЛЬНАЯ СЕТЬ                    │
         │                                                   │
...
```
Динамический пул: 192.168.1.50-199 (Split Zone)
VPN Zone:         192.168.1.20-30 (фиксированные MAC)
Bypass Zone:      192.168.1.31-49 (фиксированные MAC)
```
...
**Правила (ip rule):**
```
priority 10: from 192.168.1.20-30 → lookup vpn
priority 20: from 192.168.1.31-49 → lookup bypass
